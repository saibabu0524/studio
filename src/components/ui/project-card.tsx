...
export function ProjectCard({ project }: ProjectCardProps) {
  return (
    <Card className="flex flex-col h-full overflow-hidden shadow-lg motion-safe:hover:shadow-xl transition-all duration-300 ease-in-out motion-safe:transform motion-safe:hover:-translate-y-1 group border border-border/40">
      ...
      <CardFooter className="flex flex-col sm:flex-row justify-start gap-2 p-4 md:p-6 pt-0 mt-auto">
        <Button variant="default" size="sm" asChild className="w-full sm:w-auto shadow motion-safe:hover:shadow-md transition-shadow motion-safe:hover:-translate-y-px">
          <Link href={`/projects/${project.id}`}>
            <Eye className="mr-2 h-4 w-4" /> View Details
          </Link>
        </Button>
        {project.githubUrl && (
          <Button variant="outline" size="sm" asChild className="w-full sm:w-auto shadow motion-safe:hover:shadow-md transition-shadow motion-safe:hover:-translate-y-px">
